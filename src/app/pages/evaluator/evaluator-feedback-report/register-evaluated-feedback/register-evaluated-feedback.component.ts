...
@Component({
  selector: 'app-register-evaluated-feedback',
  templateUrl: './register-evaluated-feedback.component.html',
  styleUrls: ['./register-evaluated-feedback.component.css']
})
export class RegisterEvaluatedFeedbackComponent implements OnInit {
  EvaluatorSessionData:ILoginData = this.loginservice.GetUserSession();
  EvaluadoCodigoFicha: string = this.route.snapshot.paramMap.get('EvaluadoCodigoFicha');
  EvaluadoCodigoPuesto: string = this.route.snapshot.paramMap.get('EvaluadoCodigoPuesto');
  CodCalendario: string = this.route.snapshot.paramMap.get('CodCalendario');
  RetroalimentacionID: string = this.route.snapshot.paramMap.get('RetroalimentacionID');
  Type: string = ""
  FeedbackID: string = ""
  TipoCalendario: string = this.loginService.GetEvaluatorCalendarType(this.CodCalendario);
  ...
  async ngOnInit(): Promise<any> {
    ...
    try {
      const evaluatedValidation:any = await this.utilService.GetEvaluatorStatusForPhase('retroalimentacion','ficha',this.EvaluadoCodigoFicha,this.CodCalendario).toPromise();         
        ...
        if (this.EvaluatorSessionData.permisos[0].evaluadorJefe || this.EvaluatorSessionData.permisos[1].evaluadorJefe) {
      ...
      if (error.status === 502) {
        Swal.fire(
          ...
          this.router.navigateByUrl('/home/evaluador-reporte-retroalimentacion');
        })
      } else {
        Swal.fire(
          'Error',
  ...
  private handleFeedbackStatus(): void {
    ...
    if(registros.estadoEvaluacion){
      if(!registros.estadoEvaluador && !registros.estadoEvaluado){
        ////console.log('Evaluador va a registrar su retroalimentación')
         this.ShowEvaluatorTextField = true;
         this.DisableEvaluatorTextField = false;
         this.DisableSubmitButton = false;
         this.SubmitEvaluatorFeedback = true;
    }
    if(registros.estadoEvaluador && !registros.estadoEvaluado){
        ////console.log('El evaluador va a ver su retroalimentación')
         this.ShowEvaluatorTextField = true;
     ...
     if(registros.estadoEvaluador && registros.estadoEvaluado){
         ////console.log('El evaluador va a ver la retroalimentación del su evaluado')
         this.ShowEvaluatorTextField = true;
         this.DisableEvaluatorTextField = true;
         this.DisableSubmitButton = true;
         this.SubmitEvaluatorFeedback = true;
  ...
  private processAutoEvaluationAndEvaluationData(): void {
      ...
      this.AutoEvaluationResult=AutoEvaluationData.promedioEvaluacion
    ...
    Swal.fire({
      ...
      if (result.value) {
          ...
          let BodyToPost:ISendFeedback = 
          {
            ...
            mensaje : this.EvaluatorFeedbackText
          }
