...
@Component({
  selector: 'app-pid-watchevaluatedcompliancepid',
  templateUrl: './pid-watchevaluatedcompliancepid.component.html',
  styleUrls: ['./pid-watchevaluatedcompliancepid.component.css']
})
export class PidWatchevaluatedcompliancepidComponent implements OnInit {
 ...
 LoadDataTable(){
  ...
  MostrarModalDeAprobacion(){
    Swal.fire({
      title: "¡Espera!",
      text: `¿Tienes alguna observación?`,
      type: 'warning',
      ...
      if (result.value) { 
        Swal.fire({
          ...
          input: 'textarea',
          inputPlaceholder: 'Escriba la observación del cumplimiento...',
          showCancelButton: true,
          confirmButtonText: 'Continuar',
          cancelButtonText: 'Cancelar',
          preConfirm: (observation) => {
            if (!observation) {
              Swal.showValidationMessage('Debe de llenar el campo de observación');
              return false;
            }
            return observation;
          }
          ...
          if (result.value) { 
            this.EvaluatorFeedbackDesc = result.value;
            ////console.log('Opini:', this.EvaluatorFeedbackDesc);
            this.DarVeredicto(true, true);
          }
        });
      } else if (result.dismiss === Swal.DismissReason.cancel) { 
        this.DarVeredicto(true, false);
      }
    });    
  }
  ...
  MostrarModalDeRechazo(){
        Swal.fire({
          ...
          input: 'textarea',
          inputPlaceholder: 'Escriba la razón del rechazo del cumplimiento...',
          showCancelButton: true,
          confirmButtonText: 'Continuar',
          cancelButtonText: 'Cancelar',
          preConfirm: (observation) => {
  ...
  DarVeredicto(Veredicto: boolean, ConObservacion: boolean):any{
    ...
    ////console.log(this.DataToShow)
    let BodyToSend: IPIDComplianceRegisterVeredict;
    
    if(ConObservacion){
      BodyToSend = {
        veredictoCumplimiento: Veredicto,
        comentarioCumplimiento: this.EvaluatorFeedbackDesc
     };
    }else{
      BodyToSend = {
        veredictoCumplimiento: Veredicto,
    ...
    let descriptiontext = ConObservacion ? `¿Estás seguro de que deseas ${veredictoTexto} el cumplimiento de PID del evaluado ${this.DataToShow.registros.evaluado.apellidosNombres} con la siguiente observación?: ${this.EvaluatorFeedbackDesc}` : `¿Estás seguro de que deseas ${veredictoTexto} el cumplimiento de PID del evaluado ${this.DataToShow.registros.evaluado.apellidosNombres}?`
    ...
    Swal.fire({
      ...
      if (result.value) {
        this.utilService.showLoading();
        this.pidService.SaveEvaluatorVerdictCompliancePID(this.DataToShow.registros.evaluador.codigoFicha,this.CodFichaEvaluado,this.CodCalendario,BodyToSend).subscribe({
          next: (data) => {
            ////console.log(data)
            Swal.fire({
              title:  `Evaluación de cumplimiento de PID  ${titletext}`,
              text: `Su ${minuscula} de cumplimiento de PID fue registrado con éxito.`,
              type: 'success',
